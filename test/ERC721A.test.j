    ...
    context(`${contract}`, function () {
      ...
      describe("EIP-165 support", async function () {
        it("supports ERC165", async function () {
          expect(await this.erc721a.supportsInterface("0x01ffc9a7")).to.eq(
            true
          );
        });
      ...
      context("with minted tokens", async function () {
        beforeEach(async function () {
          ...
          await this.erc721a["safeMint(address,uint256)"](
            addr1.address,
        ...
        describe("tokenURI (ERC721Metadata)", async function () {
          describe("tokenURI", async function () {
            ...
            it("reverts when tokenId does not exist", async function () {
              await expect(
                this.erc721a.tokenURI(offsetted(this.expectedMintCount))
        ...
        describe("exists", async function () {
          it("verifies valid tokens", async function () {
            for (
              let tokenId = offsetted(0);
              tokenId < offsetted(this.expectedMintCount);
              tokenId++
            ) {
          ...
          it("verifies invalid tokens", async function () {
            expect(
              await this.erc721a.exists(offsetted(this.expectedMintCount))
            ).to.be.false;
          });
        });
        ...
        describe("balanceOf", async function () {
          ...
          it("returns correct amount with transferred tokens", async function () {
            ...
            // sanity check
            expect(await this.erc721a.ownerOf(tokenIdToTransfer)).to.equal(
              this.addr3.address
            );
        ...
        describe("_numberMinted", async function () {
          it("returns the amount for a given address", async function () {
            ...
            ).to.equal(this.addr1.expected.mintCount);
            expect(
              await this.erc721a.numberMinted(this.addr2.address)
            ).to.equal(this.addr2.expected.mintCount);
            expect(
              await this.erc721a.numberMinted(this.addr3.address)
            ).to.equal(this.addr3.expected.mintCount);
          });
        ...
        context("test transfer functionality", function () {
          const testSuccessfulTransfer = function (
            ...
            beforeEach(async function () {
              ...
              await this.erc721a
                .connect(sender)
                .approve(this.to.address, this.tokenId);

              const ownershipBefore = await this.erc721a.getOwnershipAt(
                this.tokenId
              );
              ...
              const ownershipAfter = await this.erc721a.getOwnershipAt(
                this.tokenId
              );
          ...
          const testUnsuccessfulTransfer = function (transferFn) {
            ...
            it("rejects transfer from incorrect owner", async function () {
              await this.erc721a
                .connect(this.addr2)
                .setApprovalForAll(this.sender.address, true);
              await expect(
                this.erc721a
                  .connect(this.sender)
                  [transferFn](
        ...
        describe("_burn", async function () {
          ...
          it("can burn if approvalCheck is false", async function () {
            ...
            await this.erc721a
              .connect(this.addr2)
              ["burn(uint256,bool)"](this.tokenIdToBurn, false);
          ...
          it("revert if approvalCheck is true", async function () {
            await expect(
              this.erc721a
                .connect(this.addr2)
                ["burn(uint256,bool)"](this.tokenIdToBurn, true)
            ).to.be.revertedWith("TransferCallerNotOwnerNorApproved");
          });
          ...
          it("can burn without approvalCheck parameter", async function () {
            ...
            await this.erc721a
              .connect(this.addr2)
              ["burn(uint256)"](this.tokenIdToBurn);
            expect(await this.erc721a.exists(this.tokenIdToBurn)).to.be.false;
          });
        });
        ...
        describe("_initializeOwnershipAt", async function () {
          ...
          it("doesn't set ownership if it's already setted", async function () {
            const lastTokenId = this.addr3.expected.tokens[2];
            expect(await this.erc721a.ownerOf(lastTokenId)).to.be.equal(
              this.addr3.address
            );
      ...
      context("test mint functionality", function () {
        ...
        const testSuccessfulMint = function (
          ...
          beforeEach(async function () {
            ...
            this.balanceBefore = (
              await this.erc721a.balanceOf(this.minter.address)
            ).toNumber();
            ...
            this.mintTx = await this.erc721a[mintFn](
              this.minter.address,
              quantity
            );
          });
          ...
          it("adjusts owners balances", async function () {
            expect(
              await this.erc721a.balanceOf(this.minter.address)
            ).to.be.equal(this.balanceBefore + quantity);
          });
          ...
          it("adjusts OwnershipAt and OwnershipOf", async function () {
            ...
            expect(ownership.startTimestamp).to.be.lt(
              this.timestampToMine + 10
            );
          ...
          context("mint", function () {
            context("for contract", function () {
              ...
              it("does not revert for non-receivers", async function () {
                const nonReceiver = this.erc721a;
                await this.erc721a.safeMint(1);
                expect(await this.erc721a.ownerOf(offsetted(0))).to.equal(
                  nonReceiver.address
                );
              });
            });
          ...
          context("safeMint", function () {
            context("for contract", function () {
              ...
              it("validates ERC721Received with custom _data", async function () {
                const customData =
                  ethers.utils.formatBytes32String("custom data");
                const tx = await this.erc721a[
                  "safeMint(address,uint256,bytes)"
                ](this.receiver.address, 1, customData);
                await expect(tx)
                  .to.emit(this.receiver, "Received")
                  .withArgs(
                    this.owner.address,
        ...
        context("unsuccessful mints", function () {
          ...
          context("safeMint", function () {
            ...
            it("reverts for non-receivers", async function () {
              const nonReceiver = this.erc721a;
              await expect(
                this.erc721a["safeMint(address,uint256)"](
                  nonReceiver.address,
                  1
                )
              ).to.be.revertedWith("TransferToNonERC721ReceiverImplementer");
      ...
      context("_toString", async function () {
        it("returns correct value", async function () {
          ...
          expect(await this.erc721a["toString(uint256)"](uint256Max)).to.eq(
            uint256Max
          );
        });
      });
...
describe(
  ...
  createTestSuite({
    contract: "NVCNFT",
    constructorArgs: ["Next Venture Capital", "NVC"],
  })
);
